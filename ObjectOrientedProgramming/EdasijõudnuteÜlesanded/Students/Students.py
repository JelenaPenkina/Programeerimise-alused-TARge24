...
class Student:
    ...
    def __repr__(self):
        ...
        return f"Student(name={self.name}, courses={self.courses}, grade={self.grade})"
...
def filter_by_course(student_list: list, course: str) -> list:
    ...
    """
    return list(filter(lambda s: course in s.courses, student_list))
...
def succeeding_students(student_list: list) -> list:
    ...
    """
    return list(filter(lambda s: not is_failing(s), student_list))
...
def failing_students(student_list: list) -> list:
    ...
    """
    return list(filter(lambda s: is_failing(s), student_list))
...
def sort_by_best_grade(student_list: list) -> list:
    ...
    """
    return sorted([s for s in student_list if not is_failing(s)], key=lambda s: (s.grade, s.name), reverse=True)
...
def sort_by_worst_grade(student_list: list) -> list:
    ...
    """
    return sorted([s for s in student_list if not is_failing(s)], key=lambda s: (s.grade, s.name))
